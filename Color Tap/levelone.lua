...
function new()
	...
	local display_txt = display.newText("Wait", 0, 0, "Showcard Gothic", 25*2);
	...
	display_txt.x = 25; display_txt.y = _H-450;
	
	localGroup:insert(display_txt);

	local countdowntxt = display.newText(time_remain, 0, 0, "Showcard Gothic", 25*2);
	...
	countdowntxt.x = _W-40; countdowntxt.y = _H-450;
	...
	local function spawnOrb()
		...
		transition.to(orb, {time=125, xScale = 1, yScale = 1})
	...
	local green = display.newImage("green.png");
		...
		if(o == total_orbs) then
			gametmr = timer.performWithDelay(1000, countDown, 20);
	...
	tmr = timer.performWithDelay(20, spawnPink );
