...
function new()
	...
	local background = display.newImage( "levelthree.jpg" )
	...
	physics.addBody( leftwall, "static", { density = 1.0, bounce = 2 } )
	...
	local function trackOrbs(obj)
		...
		if(time_up ~= true) then
			...
			if(o == 0) then
				director:changeScene( "congratslevelthree" )
	...
	local yellow = display.newImage("yellow.png");
		...
		transition.to(yellow, {time=150, xScale = .1, yScale = .1})
