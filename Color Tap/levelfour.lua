...
function new()
	...
	display_txt:setTextColor(255,20,147)
	...
	countdowntxt:setTextColor(255,20,147)
	...
	local function trackOrbs(obj)
		...
		if(time_up ~= true) then
			...
			if(o == 0) then
				director:changeScene( "congratslevelfive" )
	...
	local function spawnOrb()
		local orb = display.newImageRect("pink.png", 50, 50);
	...
	local yellow = display.newImage("yellow.png");
		...
		transition.to(yellow, {time=125, xScale = .1, yScale = .1})
	...
	function changeScene(e)
		if(e.phase == "ended") then
			...
			if(e.target.scene) then
				...
				tmr = timer.performWithDelay(30, spawnOrb, total_orbs);
			end
		end
	end
